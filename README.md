...
- [Configuration](#configuration)
  ...
  - [Solver (`SolverOptions`)](#solver-solveroptions)
- [Velocity model (`Homogeneous` / `NLLGrid`)](#velocity-model-homogeneous--nllgrid)
- [Saving and reloading clusters](#saving-and-reloading-clusters)
- [Cross-correlation](#cross-correlation)
  - [Waveform sources (`pyrtdd.hdd.WaveformProxy`)](#waveform-sources-pyrtddhddwaveformproxy)
  - [Caching waveforms on disk](#caching-waveforms-on-disk)
  - [Configuration (`XcorrOptions`)](#configuration-xcorroptions)
- [Relocating a single event](#relocating-a-single-event)
- [Logging](#logging)
...
5. From now on you can activate the pyrtdd environment with:
...
# (meaning higher uncertainty) therefore gets a lower weight.
...
# boundary (0.400s here), gets the worst (last) class, meaning the lowest
...
#
# Simple mode (numEllipsoids = 0): plain nearest-neighbour, picking the
...
solver_cfg.downWeightingByResidualStart = 10.  # 0 -> disable downweighting
solver_cfg.downWeightingByResidualEnd = 5.     # 0 -> disable downweighting

solver_cfg.usePickUncertainties = False  # if True then phase uncertainties must
...
```python
from pyrtdd.hdd import NLLGrid
...
Cross-correlation needs two things: a `pyrtdd.hdd.WaveformProxy` to supply waveform data, passed to `DD` as its fourth (`wf`) argument, and an `XcorrOptions` configuring cross-correlation itself, passed to `relocateMultiEvents`.
...
# Optionally save cross-correlation results to reuse them in a later call.
# If `saveProcessing=True`, a copy of this data is already stored in `processingDataDir`.
xcorr_data.writeToFile(cat, "xcorr_cache.csv")
```

`proxy` and `xcorr_cfg` are covered in detail next.

`xcorr_data` can be reused in a later call to skip recomputing cross-correlation pairs it already has entries for:

```python
# Reload xcorr results instead of recomputing them from scratch:
# `cat` must be the same background catalog the cache was computed against
...
### Waveform sources (`pyrtdd.hdd.WaveformProxy`)

`WaveformProxy` is a Python-subclassable base class: any object whose class inherits from `hdd.WaveformProxy` and implements its methods (`loadTrace`, `loadTraces`, `getComponentsInfo`, `filter`, `writeTrace`, `readTrace`) can be handed to `DD`.
...
proxy = StreamProxy(
    stream,          # the obspy.Stream to serve waveforms from
    inventory,
    guess_zne=False,
)
...
proxy = ObspyClientProxy(
    client,          # any obspy client object with a get_waveforms(...) method
    inventory,
    guess_zne=False,
)

# Scan a local folder for waveform files (any format obspy can read, e.g.
# miniSEED) and serve them like an archive. The folder is indexed once. Files added
# to the folder afterward aren't picked up. Waveform data is not held in memory.
# The construction-time scan only reads each file's header (network/station/location/channel
# and its time range), which is what gets indexed.
proxy = FileScannerProxy(
    "/path/to/waveforms",  # folder to scan
    inventory,
    guess_zne=False,
...
All three share the same base options:

- `inventory`: an `obspy.Inventory` (StationXML) used to resolve channel orientation (dip/azimuth) for the `R`/`T`/`L2` cross-correlation component transforms. `L2` is the default for S phases, so this is needed even for the default `XcorrOptions` config. Can be `None` if you don't have one; that's equivalent to an inventory with no matching entries, so orientation resolution always falls through to `guess_zne` below.
- `guess_zne` (default `False`): when a channel's orientation can't be resolved from `inventory` (including when `inventory` is `None`), fall back to assuming a standard Z/N/E layout (dip -90/0/0, azimuth 0/0/90) instead of raising. Irrelevant for phases that only ever use literal components (e.g. `"Z"`, the default for P), since those never need orientation info in the first place.

Writing your own `WaveformProxy` subclass (e.g. for a different data source, or a non-obspy pipeline) is the same shape as `pyrtdd.obspy.waveform`'s implementations: subclass `pyrtdd.hdd.WaveformProxy` and implement its six methods.

### Caching waveforms on disk

Fetching waveforms is often the slowest part of cross-correlation, and it's common for it to end up slower than the cross-correlation computation itself: `ObspyClientProxy` wrapping `obspy.clients.fdsn.Client` pays a network round trip per request, `ObspyClientProxy`/`FileScannerProxy` reading from an SDS archive or folder on a network-mounted filesystem pay network/filesystem latency on every read, and even a fast local archive can become the bottleneck once a catalog has many phases (every phase of every neighbouring event pair needs a trace fetched).

`dd.enableCatalogWaveformDiskCache(cacheDir, diskTraceMinLen=10.)` addresses this by caching every waveform `dd` fetches to `cacheDir`, and reading from there instead of re-fetching whenever the same trace is needed again:

```python
dd.enableCatalogWaveformDiskCache("/path/to/waveform_cache", diskTraceMinLen=10.)
# ... find clusters and relocate as usual ...
...
`relocateMultiEvents` also takes the `XCorrCache` argument (`xcorr_data` in the template above), used to cache computed coefficients/lags.
