...
void InitTrace(py::module_ &m) {
  ...
  // `smart_holder` is required here (not just the default unique_ptr holder):
  // WaveformProxy overrides written in Python construct and return `Trace` objects
  // (from `loadTrace`/`readTrace`, and via the `onTraceLoaded` callback in
  // `loadTraces`), and pybind11 only supports moving a Python-held instance
  // into a C++ `unique_ptr<Trace>` across that boundary with smart_holder.
  py::classh<Trace>(m, "Trace")
      .def(
