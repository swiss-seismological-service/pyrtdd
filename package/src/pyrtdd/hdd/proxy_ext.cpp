...
// Trampoline: lets `WaveformProxy` be subclassed directly in Python.
// Combined with the `smart_holder` binding below, an instance created in
// Python can have its ownership moved into a C++ `unique_ptr<Proxy>` (as
// `HDD::DD`'s constructor does) and virtual calls still dispatch back to the
// Python overrides afterwards.
class PyWaveformProxy : public Proxy, public py::trampoline_self_life_support {
public:
  using Proxy::Proxy;
  ...
  void loadTraces(
    ...
    if (!override) {
      py::pybind11_fail(
          "Tried to call pure virtual function \"WaveformProxy::loadTraces\"");
    }
    py::list pyRequest;
  ...
  void getComponentsInfo(
    ...
    if (!override) {
      py::pybind11_fail("Tried to call pure virtual function "
                        "\"WaveformProxy::getComponentsInfo\"");
    }
...
void InitWaveformProxy(py::module_ &m) {

  py::classh<Proxy, PyWaveformProxy>(m, "WaveformProxy")
      .def(py::init<>())
